...
//	TextView tv_title;
	Handler handler = new Handler(){
		public void handleMessage(Message msg){
			...
			case LOGIN:
				...
				break;
			case DISCONNECT:
				Toast.makeText(ControlDeviceActivity.this, "设备已断开", Toast.LENGTH_SHORT).show();
				finish();
				break;
			case UNBAND_FAIL:
				Toast.makeText(ControlDeviceActivity.this, "解绑失败", Toast.LENGTH_SHORT).show();
				break;
			case UPDATE_UI:
			...
			case LOG:
				...
				for(int i = 0;i<recinfo.size();i++){
					sb.append(recinfo.get(i).getName()+" "+recinfo.get(i).getValue()+"\r\n");
				}
				Toast.makeText(ControlDeviceActivity.this, sb.toString(), Toast.LENGTH_SHORT).show();
				
				break;
			case TOAST:
				String info = msg.obj +"";
				Toast.makeText(ControlDeviceActivity.this, info, Toast.LENGTH_SHORT).show();
				break;
			}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(!xpgWifiDevice.IsOnline()){
			new AlertDialog.Builder(this).setTitle("警告").setMessage("设备不在线，不可以做控制，但可以断开或解除绑定").setPositiveButton("OK", null).show();
		}
	...
	private void initData() throws JSONException {
		...
		for(int i = 0;i<sessionJsonArrays.length();i++){
			...
			for(int j = 0;j<elementjJsonArray.length();j++){
				...
				String key = elementObject.has("key")?elementObject.getString("key"):"";
				String title = elementObject.has("title")?elementObject.getString("title"):"";
				int j_min = 0;
				int j_max = 0;
				int j_step = 1;
				int maxLength = elementObject.has("maxLength")?elementObject.getInt("maxLength"):0;
				JSONObject object = new JSONObject();
				if(elementObject.has("object")){
					object = elementObject.getJSONObject("object");
					if(object.has("uint_spec")){
						JSONObject unit_json = object.getJSONObject("uint_spec");
						j_min = unit_json.has("min") ? unit_json.getInt("min"):0;
						j_max = unit_json.has("max") ? unit_json.getInt("max"):0;
						j_step = unit_json.has("step") ? unit_json.getInt("step"):1;
					}
				}
				String action = object.has("action")?object.getString("action"):"";
				JSONArray bind = object.has("bind")?object.getJSONArray("bind"):null;
				...
				if(type.equals("QBooleanElement")){
					...
					JSONArray valeus = object.has("values")?object.getJSONArray("values"):null;
					...
					JSONArray valeus = elementObject.has("values")?elementObject.getJSONArray("values"):null;
					JSONArray items = elementObject.has("items")?elementObject.getJSONArray("items"):null;
					...
					element = new QRadioElement(this, key, title, strs_values, strs_items);
					
				}else if(type.equals("QEntryElement")){
					element = new QEntryElement(this, key, title);
					String keyboardType = elementObject.has("keyboardType")?elementObject.getString("keyboardType"):"";
					String placeholder = elementObject.has("placeholder")?elementObject.getString("placeholder"):"";
					QEntryElement el = (QEntryElement)element;
					int max = j_max;
					int min = j_min;
					
					if(keyboardType.equals("PhonePad")||keyboardType.equals("NumbersAndPunctuation")){
					...
					element = new QFloatElement(this, key, title, j_min, j_max,j_step);
					((QFloatElement)element).setChangeEnable(perm.contains("W"));
			...
			int titlesize = getResources().getDimensionPixelSize(R.dimen.title_size);
			QSession qsession= new QSession(this, session.has("title")?session.getString("title"):"", elements,titlesize,height);
...
//			xpgWifiDevice.UnBindFromService(settingManager.getUserName(), settingManager.getPassword());
			...
			if(!uid.equals("")&&!token.equals("")){
				...
				xpgWifiDevice.UnbindDevice(hideuid, hidetoken);
	...
	public void onValueChange(String value, QElement element) {
		...
		if(element instanceof QMultilineElement){
			...
			it.putExtra("mac", xpgWifiDevice.GetMacAddress());
			it.putExtra("did", xpgWifiDevice.GetDid());
			it.putExtra("maxLength", ((QMultilineElement)element).getMaxLength());
			...
			while(it_params.hasNext()){
				...
				if(param.contains("_integer_part")){
					...
					if(qElement!=null){
						...
						if(valueString.contains(".")){
							String substr = valueString.substring(valueString.indexOf(".")+1);
				...
				//处理含有_decimal_part的部分
					...
					if(qElement!=null){
						String valueString = qElement.getValue();
						if(valueString.contains(".")){
							String substr = valueString.substring(0,valueString.indexOf("."));
